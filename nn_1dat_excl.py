...
def ein_start(arr, noise):
 op = np.zeros([len(arr)+noise, len(arr[0])])
 op[:len(arr), :]=arr
 op[len(arr):, :] = np.random.random([noise, len(arr[0])])*2-1
 return op

def ein_start_duo(arr, noise):
 op= np.zeros([len(arr)+2*noise, len(arr[0])])
 op[:(len(op)//2), :(len(op[0])//2)] = ein_start(arr[:(len(arr)//2),:(len(arr[0])//2)], noise)
 op[(len(op)//2):, (len(op[0])//2):] = ein_start(arr[(len(arr)//2):,(len(arr[0])//2):], noise)
 return op
...
def expand_into_noise(arr, noise1, noise2):
 ...
 op[len(arr):, len(arr[0]):] = np.random.random([noise1, noise2])*2-1
 return op
...
def expand_into_noise_switchup(arr, noise1, noise2):
 op = np.random.random([len(arr)+noise1, len(arr[0])+noise2])*2-1
 op[:len(arr),:(len(arr[0])//2)] = arr[:,:(len(arr[0])//2)]
 op[:len(arr),(len(op[0])//2):(len(op[0])//2+len(arr[0])//2)] = arr[:,(len(arr[0])//2):]
 return op
 
def ein_end(arr, noise):
 op = np.zeros([len(arr), len(arr[0])+noise])
 op[:,:len(arr[0])]=arr
 op[:,len(arr[0]):]=np.random.random([len(arr), noise])*2-1
 return op

#

def rti_start_censor(arr, noise):
 op = np.zeros([len(arr)+noise, len(arr[0])])
 op[len(arr):,:]=1
 return op

def rti_start_censor_duo(arr, noise):
 op= np.zeros([len(arr)+2*noise, len(arr[0])])
 op[:(len(op)//2), :(len(op[0])//2)] = rti_start_censor(arr[:(len(arr)//2),:(len(arr[0])//2)], noise)
 op[(len(op)//2):, (len(op[0])//2):] = rti_start_censor(arr[(len(arr)//2):,(len(arr[0])//2):], noise)
 return op

def round_two_ify_censorlvl(arr, noise1, noise2):
 op = np.zeros([(len(arr)+noise1), len(arr[0])+noise2])
 op[(len(op)-noise1):len(op), (len(op[0])-noise2):len(op[0])] = 1
 return op
...
if True:
 ...
 model[0] = ein_start_duo(model[0], NOISE)
 model[1] = expand_into_noise_duo(model[1], 1,NOISE)
 model[2] = expand_into_noise_switchup(model[2], NOISE,2)
 model[3] = ein_end(model[3], NOISE)
 
 censor[0] = rti_start_censor_duo(censor[0],NOISE)
 censor[1] = round_two_ify_censorlvl_duo(censor[1], 1,NOISE)
 censor[2] = round_two_ify_censorlvl(censor[2], NOISE,2)*0+1
 censor[3] = rti_end_censor(censor[3], NOISE)*0+1
